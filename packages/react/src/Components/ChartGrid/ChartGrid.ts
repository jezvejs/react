...
export class ChartGrid {
    ...
    convertRelToAbs(value: number) {
        return (this.props.height ?? 0) * ((value - this.viewMin) / this.viewDelta);
    }
    ...
    getY(value: number) {
        ...
        const y = (this.props.height ?? 0) + (this.props.margin ?? 0) - yAbs;
    ...
    calculate(values: number[] | object[]) {
        ...
        const { valuesMargin = 0 } = this.props;
        if (valuesMargin > 0) {
            const scale = (this.dValue / (1 - valuesMargin)) / this.viewDelta;
