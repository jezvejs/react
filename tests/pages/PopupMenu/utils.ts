...
import { defaultMenuItemProps } from './defaultProps.ts';
...
export const getButtonIdByMenu = (popupMenuId: PopupMenuId): PopupMenuButtonId | null => (
    popupMenuButtonIds[popupMenuId] ?? null
);

export const getPopupIdByButton = (menuButtonId: PopupMenuButtonId): PopupMenuId | null => {
    const entry = Object.entries(popupMenuButtonIds).find(([, value]) => (value === menuButtonId));
    return (entry?.[0] as PopupMenuId) ?? null;
};

export const getPopupIdByPage = (
    pageId: PopupMenuPageId | null | undefined,
): PopupMenuId | null => {
    const entry = Object.entries(popupMenuPageIds).find(([, value]) => (value === pageId));
    return (entry?.[0] as PopupMenuId) ?? null;
};
